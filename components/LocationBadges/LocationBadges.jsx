    ...
    return (
      <ul className="ml-1 border-l-2">
        {locations.map((location) => (
          <li
            key={location}
            className="opacity-80 px-2 relative  w-max   text-black text-sm font-mono"
          >
            <LocationMarkerIcon
              key={location}
  ...
  return (
    <ul>
      {locations.map((location) => (
        <li
          key={location}
          className="opacity-80 px-2 py-1 relative top-10 left-3 inset-x-0 w-max z-10  bg-black text-white text-sm font-mono"
        >
